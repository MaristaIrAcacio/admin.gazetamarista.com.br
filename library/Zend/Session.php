...
class Zend_Session extends Zend_Session_Abstract
{
    ...
    {
        // Domínio
        if ($_SERVER['HTTP_HOST'] == "localhost" || $_SERVER['HTTP_HOST'] == "sites.gazetamarista.com.br" || $_SERVER['HTTP_HOST'] == "192.168.1.222") {
            $urllocal = explode("/", $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
            $sessionname =  $urllocal[1];
        } else {
            $sessionname = $_SERVER['HTTP_HOST'];
        }

        // Atualiza o nome da sessão para não gerar duplicidade entre Projetos
        $userOptions['name'] = str_replace(".", "-",$sessionname);
